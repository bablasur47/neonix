import { MessageFlags, SlashCommandBuilder, ActionRowBuilder, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle, ComponentType } from 'discord.js';
...
export async function execute(interaction) {
  ...
  if (!player) {
    await interaction.reply(`${emojis.error} No music is playing.`);
    return;
  }

  let page = 0;
  await interaction.reply({ ...buildQueueEmbed(player, page), components: buildNavComponents(player, page) });
